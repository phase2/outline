...
1. Copy the slot content into the shadow DOM via a SlotController. This can sometimes break functionality from the consumer
and should only be used when the component requires specific interaction and not for simple styling.
2. The slot content can be styled via "global" CSS rules. These global rules need to be scoped by the component name.
